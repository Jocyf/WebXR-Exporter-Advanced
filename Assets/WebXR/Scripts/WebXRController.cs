...
namespace WebXR
{
    ...
    public class WebXRController : MonoBehaviour
    {
        ...
        [Tooltip("Rotation compensation for the hand")] public Vector3 fixupAngle = new Vector3(-15f, 0f, 0f); /**/
        [Tooltip("Text to write input axes values")] public Text textUI; /**/
        ...
        public float GetAxis(string action)
        {
            for (var i = 0; i < inputMap.inputs.Count; i++)
            {
                ...
                if (action == input.actionName)
                {
                    ...
                    if (!input.gamepadIsButton && axes != null) return axes[i]; /**/
                ...
                if (action == input.actionName)
                {
                    ...
                    if (!input.gamepadIsButton && axes != null)
                    {
                        if (textUI != null)
                        {
                            ...
                            for (int j = 0; j < axes.Length; j++) { textUI.text += " " + j + ": " + axes[j].ToString("0.00"); }
                        }

                        return new Vector2(axes[2], axes[3]);
                    }

                    if (!buttonStates.ContainsKey(action)) { return Vector2.zero; }
                    return Vector2.zero;
                }
            }
        ...
        void Update()
        {
            ...
            {
                _t.localPosition = mHandPos - Vector3.up * 1.0f; /**/;
                _t.localRotation = mHandRot;
            }
